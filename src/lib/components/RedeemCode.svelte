...
<script lang="ts">
	...
	function formatCode(input: string): string {
		...
		if (withPrefix.length <= 9) return `${withPrefix.slice(0, 5)}-${withPrefix.slice(5)}`;
	...
	async function handleSubmit() {
		...
		if (cleanCode.length < 13) {
			// COLOR + 8 chars
			error = 'Please enter a complete code';
			return;
		}

		if (!fingerprintStore.fingerprint) {
			error = 'Device not ready. Please try again.';
			return;
		}
		...
		try {
			...
			if (!res.ok) {
				error = data.error ?? 'Could not redeem code';
				return;
			}
...
<div class="rounded-2xl bg-lavender-50 p-4">
	<div class="mb-3 flex items-center gap-2">
		...
		<span class="font-display font-semibold text-lavender-800"> Have a code? </span>
	</div>
