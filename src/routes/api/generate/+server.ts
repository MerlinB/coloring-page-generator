...
import { consumeGeneration, getUsageForDevice } from '$lib/server/services/usage';
import type { PageFormat } from '$lib/types';

export const POST: RequestHandler = async ({ request }) => {
	const body = await request.json();
	const { fingerprint, prompt, kidFriendly, format, editMode, sourceImageData, sourcePrompt } =
		body;
	...
	if (!prompt || typeof prompt !== 'string') {
		return json({ error: 'Please tell us what you would like to color!' }, { status: 400 });
	}
	...
	if (trimmedPrompt.length > 200) {
		return json({ error: 'Description too long (max 200 characters)' }, { status: 400 });
	}
	...
	try {
		let result;
		const pageFormat: PageFormat = format === 'landscape' ? 'landscape' : 'portrait';

		if (editMode && sourceImageData) {
			result = await editColoringPage(sourceImageData, trimmedPrompt, kidFriendly ?? false, pageFormat);
		} else {
			result = await generateColoringPage(trimmedPrompt, kidFriendly ?? false, pageFormat);
		}
		...
		const finalPrompt =
			editMode && sourcePrompt ? `${sourcePrompt} (edited: ${trimmedPrompt})` : trimmedPrompt;
