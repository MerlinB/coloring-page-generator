<script lang="ts">
	...
	function openLightboxForCurrentImage() {
		...
		const index = gallery.images.findIndex((img) => img.id === gallery.currentImage?.id);
		if (index >= 0) {
			openLightbox(index);
		}
	}
	...
	async function handleGenerate(event: SubmitEvent) {
		...
		if (!fingerprintStore.fingerprint) {
			gallery.setError('Device identification not ready. Please try again.');
			return;
		}
		...
		try {
			...
			gallery.setError('Something went wrong. Please try again!');
		} finally {
...
<svelte:head>
	...
	<meta name="description" content="Create fun coloring pages for children with AI" />
</svelte:head>
...
<main class="min-h-screen bg-background px-4 py-8">
	...
	>
		...
		<div class="order-2 space-y-6 lg:sticky lg:top-8 lg:order-1 lg:self-start">
			<!-- Error Display -->
			{#if gallery.error}
				<ErrorMessage message={gallery.error} ondismiss={() => gallery.clearError()} />
			{/if}
	...
	{#if gallery.images.length > 0}
		<section class="mx-auto mt-8 max-w-7xl rounded-3xl bg-card p-6 shadow-lg">
			<h2 class="mb-4 font-display text-2xl font-bold text-coral-700">Your Coloring Pages</h2>
			<Gallery
...
<PurchasePrompt open={showPurchasePrompt} onclose={() => (showPurchasePrompt = false)} />
