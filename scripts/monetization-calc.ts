...
function calculateEffectiveRevenue(price: number): number {
  ...
  return (
    vatCustomerShare * vatCustomerRevenue +
    (1 - vatCustomerShare) * nonVatCustomerRevenue
  )
}
...
function calculateWeightedARPU(
  packMetrics: ReturnType<typeof calculatePackMetrics>,
): number {
...
function calculateBreakEvenForFreeTier(
...
function main() {
  ...
  console.log(
    `API Cost per Image:     ${formatCurrency(CONFIG.apiCostPerImage)}`,
  )
  console.log(
    `Free Tier:              ${CONFIG.freeTier.imagesPerWeek} images/week`,
  )
  console.log(
    `VAT Customer Share:     ${formatPercent(CONFIG.vat.vatCustomerShare)}`,
  )
  console.log(
    `Average VAT Rate:       ${formatPercent(CONFIG.vat.averageVatRate)}`,
  )
  ...
  const imagesPerMonth =
    CONFIG.freeTier.imagesPerWeek * CONFIG.freeTier.weeksPerMonth
  const costPerUser = imagesPerMonth * CONFIG.apiCostPerImage
  ...
  for (const dau of CONFIG.dauScenarios) {
    ...
    console.log(
      `  ${dau.toLocaleString()} DAU: ${formatPercent(breakEven)} ${status}`,
    )
  }
  ...
  // Header
  const convHeaders = CONFIG.conversionRates
    .map((r) => formatPercent(r).padStart(10))
    .join("")
...
`)
  ...
  for (const cost of altCosts) {
    ...
    console.log(
      `  ${formatCurrency(cost)}/img: ${formatPercent(breakEven)} conversion needed${marker}`,
    )
  }
  ...
  for (const freePerWeek of CONFIG.freeTierScenarios) {
    const costPerMonth =
      freePerWeek * CONFIG.freeTier.weeksPerMonth * CONFIG.apiCostPerImage
    const breakEven = calculateBreakEvenForFreeTier(freePerWeek, weightedARPU)
    const marker =
      freePerWeek === CONFIG.freeTier.imagesPerWeek ? " (current)" : ""
  ...
  console.log(
    `${"Imgs/Week".padEnd(12)}${"Free Cost".padStart(12)}${"Revenue".padStart(12)}${"Net".padStart(12)}`,
  )
